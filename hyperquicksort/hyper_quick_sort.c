...
int main(int argc, char ** argv)
{
	...
	while (1) {
		...
		quick_sort(recv_buff, 0, scatter_cnt[comm_rank] - 1);

		MPI_Barrier(MPI_COMM_WORLD);
		printf("Rank #%-2d (local sort): %2d | %s\n", comm_rank, scatter_cnt[comm_rank], recv_buff);
...
void parallel_quick_sort(char * data_arr, const int start_index, const int end_index, MPI_Comm comm)
{
	...
	if (comm_rank < rank_pivot) {
		...
		recv_buff = malloc(sizeof(char) * exchange_count);
		MPI_Recv(recv_buff, exchange_count, MPI_CHAR, (comm_rank + rank_pivot), 0, comm, MPI_STATUS_IGNORE);
		i = 0;
		j = 0;
 		k = 0;
		while (1) {
			if ((i <= pivot) && (j < exchange_count)) {
				if (data_arr[i] <= recv_buff[j]) {
					new_arr[k++] = data_arr[i++];
				} else {
					new_arr[k++] = recv_buff[j++];
				}
			} else if (i <= pivot) {
				for (; i <= pivot; i++)
					new_arr[k++] = data_arr[i];
		...
		j = 0;
 		k = 0;
		while (1) {
			if ((i < index_cnt) && (j < exchange_count)) {
				if (data_arr[i] <= recv_buff[j]) {
					new_arr[k++] = data_arr[i++];
				} else {
					new_arr[k++] = recv_buff[j++];
